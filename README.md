...
| URL of the SSMTP Configuration File | SSMTPConfiguration | Mandatory | Input a URL to a `ssmtp.conf` file. The `ssmtp.conf` file shall contains all SSMTP configuration except credential information (i.e. User-ID and Password). Reference: [Configuring SSMTP to Send Locally Generated eMail Messages Through the ssmtp.conf File](https://support.f5.com/csp/article/K13180). |
| CFT Label | Parameter Name | Required | Description |
