...
| Upgrade Image's File Name, of the .iso file used to upgrade the Big-IP image | UpgradeImageFileName | Optional with Default Value | File Name of the upgrade `.iso` image. Note that on the same folder the `.iso.md5` checksum file of the `.iso` file MUST exist. The CloudFormation template will download BOTH `.iso` and `.iso.md5` files. Missing `.iso.md5` will render the upgrade process fail. So for example if the URL of the files are:<br>https://domain.tld/path/filename.iso<br>https://domain.tld/path/filename.iso.md5<br>Input only the filename of the `.iso` file for this parameter:<br>[filename.iso](filename.iso) |
