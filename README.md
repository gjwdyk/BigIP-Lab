...
| URL of the SSMTP Configuration File | SSMTPConfiguration | Mandatory | Input a URL to a `ssmtp.conf` file. The `ssmtp.conf` file shall contains all SSMTP configuration except credential information i.e. `AuthUser` and `AuthPass` fields to be excluded out from `ssmtp.conf` file. The `AuthUser` and `AuthPass` fields will be delivered separately in this CloudFormation as SMTPUserID and SMTPUserPassword parameter). Reference: [Configuring SSMTP to Send Locally Generated eMail Messages Through the ssmtp.conf File](https://support.f5.com/csp/article/K13180). |
| CFT Label | Parameter Name | Required | Description |
